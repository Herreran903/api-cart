...
import static com.api_cart.cart.infra.cart.util.CartSwaggerMessages.INCREASE_CART_REQUEST_EXAMPLE;
...
public class CartController {
    ...
    @Operation(
            ...
            requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    ...
                    content = @Content(
                            ...
                            examples = {
                                    @ExampleObject(value = INCREASE_CART_REQUEST_EXAMPLE)
                            }
                    )
            )
    )
    ...
    @PreAuthorize("hasRole(T(com.api_cart.cart.domain.role.util.RoleEnum).ROLE_CLIENT.toString())")
    ResponseEntity<Void> deleteArticleToCart(){
        return ResponseEntity.ok().build();
    }
}
